    ...
    public String getMessageTable(Message[] messages) {
        ...
        for (int i = messages.length - 1; i >= 0; i--) {
            ...
            buffer.append("<tr> "
                    ...
                    + " <td id=relay>"
                    + "<a href=relay_write_mail.jsp?="
 //                   + "<a href=write_mail.jsp?msgid="
                    + CommandType.SEND_MAIL_COMMAND
                    + "&msgid=" + (i + 1) + "> 전달 </a>" + "</td>" 
                            
                    + " <td id=reply>"
                    + "<a href=reply_write_mail.jsp?="
                    + CommandType.SEND_MAIL_COMMAND
                    + "&msgid=" + (i + 1) + "> 답장 </a>"+ "</td>" 
...
//        return "MessageFormatter 테이블 결과";
    ...
    public String getFromAddr(Message message){
        StringBuilder buffer = new StringBuilder();
        ...
        request.setAttribute(parser.getToAddress(),this);
